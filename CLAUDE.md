...
**Rust for Rustaceans skills (MANDATORY):** When working on Rust code, ALWAYS load and follow the matching `rfr-*` skill before writing or reviewing code. These are non-negotiable — invoke via the Skill tool, not optional suggestions.

| Topic | Skill to load |
|-------|---------------|
| Ownership, borrowing, lifetimes, drop order, interior mutability | `rfr-foundations` |
| Type layout, alignment, Sized, traits, PhantomData, marker traits | `rfr-types` |
| Public API design, function signatures, standard traits, builders | `rfr-designing-interfaces` |
| Error types, thiserror/anyhow, Display, ? operator, when to panic | `rfr-error-handling` |
| Crates, modules, workspaces, features, conditional compilation | `rfr-project-structure` |
| Unit/integration/doc tests, proptest, fuzzing, Miri, benchmarks | `rfr-testing` |
| macro_rules!, procedural macros, syn/quote, hygiene, cargo expand | `rfr-macros` |
| Futures, Pin, Waker, async/await, spawn, select!, cancellation | `rfr-async-programming` |
| unsafe blocks, raw pointers, MaybeUninit, UnsafeCell, soundness | `rfr-unsafe-code` |
| Send/Sync, Mutex, channels, atomics, ordering, deadlock prevention | `rfr-concurrency` |
| extern "C", CStr/CString, repr(C), bindgen/cbindgen, panic safety | `rfr-ffi` |
| Type state, newtype, extension traits, RAII, iterator patterns | `rfr-ecosystem` |
